    ...
    <div class="space-y-5">
        ...
        @if ($journal->journal_type === 'hafalan')
            <div class="rounded-lg bg-white p-6 shadow-[0px_1px_2px_0px_rgba(0,0,0,0.05)] outline outline-1 outline-offset-[-1px] outline-stone-300">
                ...
                <div class="grid grid-cols-1 gap-4 md:grid-cols-3">
                    <div>
                        <p class="text-xs font-semibold uppercase text-neutral-500">Jenis Hafalan</p>
                        <p class="mt-1 text-base text-zinc-900">{{ $journal->memorization_type ?? '-' }}</p>
                    </div>
                    <div>
                        <p class="text-xs font-semibold uppercase text-neutral-500">Target Hafalan</p>
                        <p class="mt-1 text-base text-zinc-900">{{ $journal->memorization_target ?? '-' }}</p>
                    </div>
                    <div>
                        <p class="text-xs font-semibold uppercase text-neutral-500">Capaian Hafalan</p>
                        <p class="mt-1 text-base text-zinc-900">{{ $journal->memorization_result ?? '-' }}</p>
                    </div>
                </div>
            </div>
        @endif
        ...
        @if ($journal->journal_type === 'legalisir_kitab')
            <div class="rounded-lg bg-white p-6 shadow-[0px_1px_2px_0px_rgba(0,0,0,0.05)] outline outline-1 outline-offset-[-1px] outline-stone-300">
                ...
                <div class="grid grid-cols-1 gap-4 md:grid-cols-3">
                    <div>
                        <p class="text-xs font-semibold uppercase text-neutral-500">Nama Kitab</p>
                        <p class="mt-1 text-base text-zinc-900">{{ $journal->kitab_name ?? '-' }}</p>
                    </div>
                    <div>
                        <p class="text-xs font-semibold uppercase text-neutral-500">Halaman</p>
                        <p class="mt-1 text-base text-zinc-900">{{ $journal->kitab_page ?? '-' }}</p>
                    </div>
                    <div>
                        <p class="text-xs font-semibold uppercase text-neutral-500">Status Legalisir</p>
                        <p class="mt-1 text-base text-zinc-900">{{ $journal->legalization_status ?? '-' }}</p>
                    </div>
                </div>
            </div>
        @endif
        ...
        @if (in_array($journal->journal_type, ['nilai_harian', 'tamrinan']))
            <div class="rounded-lg bg-white p-6 shadow-[0px_1px_2px_0px_rgba(0,0,0,0.05)] outline outline-1 outline-offset-[-1px] outline-stone-300">
                ...
                <div class="grid grid-cols-1 gap-4 md:grid-cols-3">
                    <div>
                        <p class="text-xs font-semibold uppercase text-neutral-500">Nilai Harian</p>
                        <p class="mt-1 text-base text-zinc-900">{{ $journal->daily_score ?? '-' }}</p>
                    </div>
                    <div>
                        <p class="text-xs font-semibold uppercase text-neutral-500">Nilai Ujian / Tamrinan</p>
                        <p class="mt-1 text-base text-zinc-900">{{ $journal->exam_score ?? '-' }}</p>
                    </div>
                    <div>
                        <p class="text-xs font-semibold uppercase text-neutral-500">Predikat</p>
                        <p class="mt-1 text-base text-zinc-900">{{ $journal->predicate ?? '-' }}</p>
                    </div>
                </div>
            </div>
        @endif
        ...
        <div class="flex items-center gap-3">
            ...
            <a href="{{ route('teacher.journals.student', [
                ...
                'teaching_assignment_id' => $journal->teaching_assignment_id,
            ]) }}"
