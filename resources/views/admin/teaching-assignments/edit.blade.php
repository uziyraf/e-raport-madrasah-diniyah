<x-app-layout>
    <x-slot name="header">
        <h2 class="text-xl font-bold text-teal-950">Edit Penugasan Guru Fan</h2>
    </x-slot>
    ...
    <div class="rounded-lg bg-white p-6 shadow-[0px_1px_2px_0px_rgba(0,0,0,0.05)] outline outline-1 outline-offset-[-1px] outline-stone-300">
        <form action="{{ route('admin.teaching-assignments.update', $assignment) }}" method="POST">
            @csrf
            @method('PUT')
            <div class="space-y-5">
                <div class="grid grid-cols-1 gap-5 md:grid-cols-2">
                    ...
                    <div>
                        ...
                        @error('subject_id') <p class="mt-1 text-sm text-red-700">{{ $message }}</p> @enderror
                    </div>
