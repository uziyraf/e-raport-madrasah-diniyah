<x-app-layout>
    <x-slot name="header">
        <h2 class="text-xl font-bold text-teal-950">Guru Fan/Mapel</h2>
    </x-slot>
                ...
                <tbody class="divide-y divide-stone-300">
                    @forelse ($assignments as $assignment)
                        <tr>
                            ...
                            <td class="border-t border-stone-300 px-6 py-4 text-base font-normal text-zinc-900">{{ $assignment->subject->name }}</td>
                            ...
                            <td class="border-t border-stone-300 px-6 py-4">
                                <div class="flex items-center gap-2">
                                    ...
                                    <form action="{{ route('admin.teaching-assignments.destroy', $assignment) }}" method="POST" onsubmit="return confirm('Hapus penugasan ini?')">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit"
                    ...
                    @empty
                        <tr>
                            <td colspan="7" class="px-6 py-10 text-center text-sm text-neutral-500">Belum ada data penugasan guru fan.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
