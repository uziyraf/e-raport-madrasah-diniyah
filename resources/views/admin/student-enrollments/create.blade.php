<x-app-layout>
    <x-slot name="header">
        <h2 class="text-xl font-bold text-teal-950">Tambah Penempatan Santri</h2>
    </x-slot>
                    ...
                    <div>
                        ...
                        <select name="school_class_id" id="school_class_id"
                            ...
                            @foreach ($schoolClasses as $id => $label)
                                <option value="{{ $id }}" {{ old('school_class_id') == $id ? 'selected' : '' }}>{{ $label }}</option>
                            @endforeach
                        </select>
                ...
                <div class="grid grid-cols-1 gap-5 md:grid-cols-2">
                    <div>
                        ...
                        <select name="enrollment_status" id="enrollment_status"
                                class="w-full rounded-sm border border-stone-300 bg-white px-4 py-3 text-sm text-zinc-900 outline-none transition focus:border-teal-950 focus:ring-2 focus:ring-teal-950/10">
                            <option value="active" {{ old('enrollment_status', 'active') === 'active' ? 'selected' : '' }}>Aktif</option>
                            <option value="transfer" {{ old('enrollment_status') === 'transfer' ? 'selected' : '' }}>Pindah</option>
                            <option value="graduate" {{ old('enrollment_status') === 'graduate' ? 'selected' : '' }}>Lulus</option>
                            <option value="dropout" {{ old('enrollment_status') === 'dropout' ? 'selected' : '' }}>Keluar</option>
                        </select>
