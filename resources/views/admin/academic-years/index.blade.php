<x-app-layout>
    <x-slot name="header">
        <h2 class="text-xl font-bold text-teal-950">Tahun Ajaran</h2>
    </x-slot>
            ...
            <table class="min-w-full divide-y divide-stone-300">
                ...
                <tbody class="divide-y divide-stone-300">
                    @forelse ($academicYears as $academicYear)
                        <tr>
                            <td class="border-t border-stone-300 px-6 py-4 text-base font-normal text-zinc-900">{{ $academicYear->name }}</td>
                            <td class="border-t border-stone-300 px-6 py-4 text-base font-normal text-zinc-900">{{ $academicYear->start_date->format('d/m/Y') }}</td>
                            <td class="border-t border-stone-300 px-6 py-4 text-base font-normal text-zinc-900">{{ $academicYear->end_date->format('d/m/Y') }}</td>
                            ...
                            <td class="border-t border-stone-300 px-6 py-4">
                                <div class="flex items-center gap-2">
                                    <a href="{{ route('admin.semesters.index') }}?academic_year_id={{ $academicYear->id }}"
                                    ...
                                    <form action="{{ route('admin.academic-years.destroy', $academicYear) }}" method="POST" onsubmit="return confirm('Nonaktifkan tahun ajaran ini?')">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit"
                    ...
                    @empty
                        <tr>
                            <td colspan="5" class="px-6 py-10 text-center text-sm text-neutral-500">Belum ada data tahun ajaran.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
