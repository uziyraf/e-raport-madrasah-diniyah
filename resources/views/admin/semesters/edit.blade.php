<x-app-layout>
    <x-slot name="header">
        <h2 class="text-xl font-bold text-teal-950">Edit Semester</h2>
    </x-slot>
    ...
    <div class="rounded-lg bg-white p-6 shadow-[0px_1px_2px_0px_rgba(0,0,0,0.05)] outline outline-1 outline-offset-[-1px] outline-stone-300">
        <form action="{{ route('admin.semesters.update', $semester) }}" method="POST" class="space-y-5">
            ...
            <div>
                ...
                <select name="academic_year_id" id="academic_year_id"
                    ...
                    @foreach ($academicYears as $academicYear)
                        <option value="{{ $academicYear->id }}" {{ old('academic_year_id', $semester->academic_year_id) == $academicYear->id ? 'selected' : '' }}>{{ $academicYear->name }}</option>
                    @endforeach
                </select>
            ...
            <div>
                ...
                <input type="date" name="start_date" id="start_date" value="{{ old('start_date', $semester->start_date->format('Y-m-d')) }}"
                       class="w-full rounded-sm border border-stone-300 bg-white px-4 py-3 text-sm text-zinc-900 outline-none transition focus:border-teal-950 focus:ring-2 focus:ring-teal-950/10">
            ...
            <div>
                ...
                <input type="date" name="end_date" id="end_date" value="{{ old('end_date', $semester->end_date->format('Y-m-d')) }}"
                       class="w-full rounded-sm border border-stone-300 bg-white px-4 py-3 text-sm text-zinc-900 outline-none transition focus:border-teal-950 focus:ring-2 focus:ring-teal-950/10">
            ...
            <div>
                <label for="is_active" class="inline-flex items-center gap-3">
                    ...
                    <input type="checkbox" name="is_active" id="is_active" value="1" {{ old('is_active', $semester->is_active) ? 'checked' : '' }}
