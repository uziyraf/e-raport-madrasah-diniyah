<x-app-layout>
    <x-slot name="header">
        <h2 class="text-xl font-bold text-teal-950">Edit Fan/Mapel</h2>
    </x-slot>
    ...
    <div class="rounded-lg bg-white p-6 shadow-[0px_1px_2px_0px_rgba(0,0,0,0.05)] outline outline-1 outline-offset-[-1px] outline-stone-300">
        <form action="{{ route('admin.subjects.update', $subject) }}" method="POST" class="space-y-5">
            @csrf
            @method('PUT')

            <div>
                <label for="name" class="mb-2 block text-sm font-medium text-neutral-700">Nama Fan/Mapel</label>
            ...
            <div>
                ...
                <input type="text" name="arabic_name" id="arabic_name" value="{{ old('arabic_name', $subject->arabic_name) }}" lang="ar" dir="rtl"
                       class="w-full rounded-sm border border-stone-300 bg-white px-4 py-3 text-sm text-zinc-900 outline-none transition focus:border-teal-950 focus:ring-2 focus:ring-teal-950/10">
            ...
            <div>
                ...
                <input type="number" name="sort_order" id="sort_order" value="{{ old('sort_order', $subject->sort_order) }}"
                       class="w-full rounded-sm border border-stone-300 bg-white px-4 py-3 text-sm text-zinc-900 outline-none transition focus:border-teal-950 focus:ring-2 focus:ring-teal-950/10">
