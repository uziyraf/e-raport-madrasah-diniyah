<x-app-layout>
    <x-slot name="header">
        <h2 class="text-xl font-bold text-teal-950">Tambah Jadwal Pelajaran</h2>
    </x-slot>
            ...
            <div class="space-y-5">
                    ...
                    <div>
                        ...
                        <select name="tahun_ajaran_id" id="tahun_ajaran_id"
                            ...
                            @foreach ($academicYears as $id => $name)
                                <option value="{{ $id }}" {{ old('tahun_ajaran_id') == $id ? 'selected' : '' }}>{{ $name }}</option>
                            @endforeach
                        </select>
                    ...
                    <div>
                        ...
                        <select name="kelas_id" id="kelas_id"
                            ...
                            @foreach ($schoolClasses as $id => $label)
                                <option value="{{ $id }}" {{ old('kelas_id') == $id ? 'selected' : '' }}>{{ $label }}</option>
                            @endforeach
                        </select>
                    ...
                    <div>
                        ...
                        <select name="mapel_id" id="mapel_id"
                            ...
                            @foreach ($subjects as $id => $name)
                                <option value="{{ $id }}" {{ old('mapel_id') == $id ? 'selected' : '' }}>{{ $name }}</option>
                            @endforeach
                        </select>
                    ...
                    <div>
                        ...
                        <select name="guru_id" id="guru_id"
                            ...
                            @foreach ($teachers as $id => $name)
                                <option value="{{ $id }}" {{ old('guru_id') == $id ? 'selected' : '' }}>{{ $name }}</option>
                            @endforeach
                        </select>
                ...
                <div>
                    ...
                    <textarea name="keterangan" id="keterangan" rows="3"
                              class="w-full rounded-sm border border-stone-300 bg-white px-4 py-3 text-sm text-zinc-900 outline-none transition focus:border-teal-950 focus:ring-2 focus:ring-teal-950/10 @error('keterangan') border-red-400 @enderror">{{ old('keterangan') }}</textarea>
