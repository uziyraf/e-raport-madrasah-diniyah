<x-app-layout>
    <x-slot name="header">
        <h2 class="text-xl font-bold text-teal-950">Data Guru</h2>
    </x-slot>
    ...
    <div class="space-y-5">
        ...
        <div class="flex items-center justify-between">
            <form method="GET" action="{{ route('admin.teachers.index') }}" class="flex items-center gap-2">
                <input type="text" name="search" value="{{ request('search') }}" placeholder="Cari nama atau kode guru..."
            ...
            <table class="min-w-full divide-y divide-stone-300">
                ...
                <tbody class="divide-y divide-stone-300">
                    @forelse ($teachers as $teacher)
                        <tr>
                            <td class="border-t border-stone-300 px-6 py-4 text-base font-normal text-zinc-900">{{ $teacher->teacher_code ?? '-' }}</td>
                            <td class="border-t border-stone-300 px-6 py-4 text-base font-normal text-zinc-900">{{ $teacher->name }}</td>
                            <td class="border-t border-stone-300 px-6 py-4 text-base font-normal text-zinc-900">{{ $teacher->gender === 'male' ? 'Laki-laki' : ($teacher->gender === 'female' ? 'Perempuan' : '-') }}</td>
                            <td class="border-t border-stone-300 px-6 py-4 text-base font-normal text-zinc-900">
                                @if ($teacher->phone)
                                    <span class="block text-sm">{{ $teacher->phone }}</span>
                                @endif
                                @if ($teacher->email)
                                    <span class="block text-sm text-neutral-500">{{ $teacher->email }}</span>
                                @endif
                            ...
                            <td class="border-t border-stone-300 px-6 py-4">
                                <div class="flex items-center gap-2">
                                    ...
                                    <form action="{{ route('admin.teachers.destroy', $teacher) }}" method="POST" onsubmit="return confirm('Nonaktifkan guru ini?')">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit"
                    ...
                    @empty
                        <tr>
                            <td colspan="7" class="px-6 py-10 text-center text-sm text-neutral-500">Belum ada data guru.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
